...
def mobile(request,brand_name=None):
    if brand_name == None:
        ...
        brand_name = Product.objects.filter(category="M").filter(brand=brand_name)
...
def orders(request):
    if request.user.is_authenticated:
        ...
        orders=[]
        for ob in obj:
            orders.append([ob,ob.quantity*Product.objects.get(id=ob.product.id).discount_price])
...
class ProductDetailsView(View):
    def get(self,request,pk):
        ...
        if request.user.is_authenticated:
            is_inside_cart = Cart.objects.filter(user=request.user).filter(product=product)
...
def cart(request):
    ...
    product_amount = sum([prod.product.discount_price*prod.quantity for prod in Cart.objects.filter(user=request.user)])
...
def alter_quantity(request):
    if request.user.is_authenticated:
        if request.GET.get('action_')=='increase':
            obj = Cart.objects.get(Q(user=request.user) & Q(product=Product.objects.get(id=request.GET.get('product_id'))))
            obj.quantity+=1
            obj.save()
        else:
            obj = Cart.objects.get(Q(user=request.user) & Q(product=Product.objects.get(id=request.GET.get('product_id'))))
